...
# Ventas Perdidas
# -----------------------------
@app.route("/ventas_perdidas", methods=["GET", "POST"])
@requiere_acceso("ventas_perdidas")
def ventas_perdidas():
    cursor = get_cursor()
    fecha_hoy = date.today().strftime("%Y-%m-%d")

    # --- Inserción de registros desde formulario ---
    if request.method == "POST":
        ...
        # Validación backend: ningún campo vacío
        ...
        return redirect(url_for("ventas_perdidas"))
    ...
    # --- Lista de vendedores únicos para el combo ---
...
# Editar registro
# -----------------------------
@app.route("/editar_venta_perdida/<int:id>", methods=["GET", "POST"])
...
# Eliminar registro
# -----------------------------
@app.route("/eliminar_venta_perdida/<int:id>")
...
# -----------------------------
@app.route("/importar_ventas_perdidas", methods=["POST"])
